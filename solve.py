    ...
    def train(self):
        ...
        while True:
            ...
            if delta < self.threshold:
                break
            training_step += 1
            print(f"第{training_step}次训练,delta= {delta}")
...
class McExploring(Solve):
    def __init__(self, policy):
        super().__init__(policy)
        self.num_episodes = 30  # 训练次数
        self.length_of_episodes = 10  # 每次训练的episode长度
        self.epsilon = 0.1  # epsilon贪心策略

    def one_episode(self, start_state, start_action):
        """
        ...
        next_state, reward, done, _ = self.policy.env.step(start_action)
        state_action_rewards[0][0] = state[0]
        state_action_rewards[0][1] = state[1]
        state_action_rewards[0][2] = start_action
        state_action_rewards[0][3] = reward
        state = next_state
        for i in range(self.length_of_episodes-1):
            action = self.policy.sample_action(state)
            next_state, reward, done, _ = self.policy.env.step(action)
            state_action_rewards[i + 1][0] = state[0]
            state_action_rewards[i + 1][1] = state[1]
            state_action_rewards[i + 1][2] = action
            state_action_rewards[i + 1][3] = reward
            state = next_state
        return state_action_rewards

    def train(self):
        # 初始化,把action value 初始化成 负无穷
        step = 0
        self.action_values = np.full((self.policy.env.size, self.policy.env.size, self.policy.env.action_space.n),
                                        -np.inf)
        while True:
            previous_action_values = self.action_values.copy()
            for x in range(self.policy.env.size):
                for y in range(self.policy.env.size):
                    for action in range(self.policy.env.action_space.n):
                        start_state = (x, y)
                        # 生成一个episode
                        Num = np.zeros((self.policy.env.size, self.policy.env.size, self.policy.env.action_space.n),
                                       dtype=int)
                        returns = np.zeros((self.policy.env.size, self.policy.env.size, self.policy.env.action_space.n))
                        for _ in range(self.num_episodes + 1):
                            state_action_rewards = self.one_episode(start_state, start_action=action)
                            g = 0
            ...
            if delta < self.threshold:
                break
            print(f"第{step}次训练完成,delta = {delta}")
            step += 1
            self.epsilon *= 0.9
...
class Sarsa(Solve):
    ...
    def one_episode(self):
        self.policy.env.reset()
        st = self.policy.env.state
        at = self.policy.sample_action(st)
        while self.policy.env.state != self.policy.env.target:
            # collect r_t+1 and s_t+1, a_t+1
            st1, rt1, done, _ = self.policy.env.step(at)
            at1 = self.policy.sample_action(st1)
            # update Q for (s_t, a_t)
            self.action_values[st[0], st[1], at] += self.alpha * (
                    rt1 + self.discounted_rate * self.action_values[st1[0], st1[1], at1] - self.action_values[st[0], st[1], at])
            # update policy for s_t
            best_action = np.argmax(self.action_values[st[0], st[1]])
            self.policy.probs[st[0], st[1]] = np.zeros(self.policy.env.action_space.n)
            self.policy.probs[st[0], st[1]][best_action] = 1 - self.epsilon
            self.policy.probs[st[0], st[1]] += self.epsilon / self.policy.env.action_space.n
            st = st1
            at = at1
